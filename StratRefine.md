...
$R_{\text{preference}}$ 使用人工动作权重表达特定战略路线下的条件性先验。设动作 $a$ 在生成结果和参考答案中的数量分别为 $C_g(a)$ 与 $C_r(a)$，人工权重为 $W(a)$，参考动作总数为 $N_r$，则：
...
正权重鼓励支持目标科技路线的动作，负权重抑制明显偏离该路线的资源投入。这些权重只表示特定策略条件下的训练先验，不代表单位或建筑在所有状态中的固定价值。
