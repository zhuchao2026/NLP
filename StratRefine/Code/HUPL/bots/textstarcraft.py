...
class ChatBot_SingleTurn:
    ...
    def __init__(self, model_name, api_key, api_base, system_prompt, example_prompt, temperature, provider="openai", model_path="models/STR"):
        """
        ...
        self.chat = create_openai_client(
            api_key=api_key,
            provider=provider,
            base_url=api_base,
    ...
    def query(self, user_input, max_retries=5):
        ...
        while True:
            try:
                ...
                if any(token in error_text for token in ["invalid api key", "incorrect api key", "authentication", "unauthorized", "401"]):
                    raise RuntimeError(
                        f"LLM authentication failed. base_url={self.chat.base_url}, "
                        f"model={self.model_name}, error={e}"
                    ) from e
                time.sleep(5)
...
class L2_summary:
    ...
    def __init__(self, LLMapi_base, LLMapi_key, model_name, temperature, system_prompt, example_prompt, chunk_window, prompt_type, provider="openai", model_path="models/STR"):
        """
        初始化
        :param LLMapi_base:
        :param LLMapi_key:
        ...
        self.chatbot = ChatBot_SingleTurn(api_base=self.api_base, api_key=self.api_key, model_name=self.model_name,
                                          ...
                                          example_prompt=self.example_prompt, provider=provider, model_path=model_path)
...
class TextStarCraft(BotAI):
    def __init__(self, args, hep=False):
        ...
        self.api_base = args.LLM_base_url
        ...
        self.L2 = L2_summary(LLMapi_base=self.api_base, LLMapi_key=self.api_key, model_name=self.model_name,
                             ...
                             prompt_type='v4', provider=args.LLM_provider, model_path=args.LLM_model_path)
        self.game_folder = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', args.save_path, 'agent')
